...
- **Основа для нескольких диалектов** - PostgreSQL и SQLite поддерживаются через диалектные адаптеры
...
|----------|---------|-------------|
| `DATABASE_DIALECT` | `postgres` | Диалект БД (`postgres`, `sqlite`) |
| `DATABASE_DSN` | — | Строка подключения к базе данных |
| `MIGRATIONS_DIR` | `migrations` | Директория миграций |
...
- Go 1.24 или новее
- PostgreSQL 12 или новее (для диалекта postgres)
- SQLite 3 или новее (для диалекта sqlite)
...
| Создание таблицы | ✅ | ✅ |
| Добавление колонки | ✅ | ✅ |
| Удаление/изменение колонки | ✅ | ✅ (авто-пересборка таблицы) |
| Первичный/уникальный ключ | ✅ | ✅ (авто-пересборка таблицы) |
| Внешние ключи | ✅ | ✅ (прямо + через авто-пересборку) |
| Индексы | ✅ | ✅ |
| CHECK-ограничения | ✅ | ✅ (прямо + через авто-пересборку) |
| Детерминированный порядок diff | ✅ | ✅ |

Для SQLite несовместимые `ALTER TABLE` изменения автоматически планируются как детерминированная пересборка таблицы (временная таблица, копирование данных, подмена таблицы, пересоздание индексов). При генерации также выводятся предупреждения о best-effort маппинге типов, например `uuid/jsonb/timestamptz -> TEXT` и `boolean -> INTEGER`.
